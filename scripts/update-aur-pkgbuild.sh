...
# Rewrite sha256sums block (AppImage, desktop, icon)
python3 - "$PKGBUILD" "$SHA_APP" "$SHA_DESK" "$SHA_ICON" <<'PY'
import pathlib, re, sys
path, sha_app, sha_desk, sha_icon = pathlib.Path(sys.argv[1]), sys.argv[2], sys.argv[3], sys.argv[4]
...
if command -v makepkg >/dev/null 2>&1; then
  ...
  cat > "$DIR/.SRCINFO" <<EOF
pkgbase = tenling-bin
	pkgdesc = TenLing — calm, cross-platform Markdown viewer and editor
	pkgver = ${VERSION}
	pkgrel = 1
