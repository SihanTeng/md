...
#   md-{version}-linux-x64.deb
#   md-{version}-linux-x64.rpm
#
# Writes:
...
DEB="$ASSETS/md-${VERSION}-linux-x64.deb"
RPM="$ASSETS/md-${VERSION}-linux-x64.rpm"
...
if [[ -f "$DEB" ]]; then
  ...
  cp -f "$DEB" "$OUT/apt/md_${VERSION}_amd64.deb"
  cp -f "$DEB" "$OUT/apt/md-${VERSION}-linux-x64.deb"
  (
    cd "$OUT/apt"
    if command -v dpkg-scanpackages >/dev/null 2>&1; then
      ...
      SIZE=$(stat -c%s "md_${VERSION}_amd64.deb")
      SHA256=$(sha256sum "md_${VERSION}_amd64.deb" | cut -d' ' -f1)
      MD5=$(md5sum "md_${VERSION}_amd64.deb" | cut -d' ' -f1)
      cat > Packages <<EOF
Package: md
Version: ${VERSION}
Architecture: amd64
Maintainer: md contributors
Filename: ./md_${VERSION}_amd64.deb
Size: ${SIZE}
SHA256: ${SHA256}
MD5sum: ${MD5}
Section: editors
...
Architectures: amd64
Components: main
Description: md APT repository (GitHub Pages)
Date: $(date -Ru)
EOF
  )
...
if [[ -f "$RPM" ]]; then
  cp -f "$RPM" "$OUT/rpm/md-${VERSION}-linux-x64.rpm"
  # Also a more DNF-friendly NEVRA-ish name
  cp -f "$RPM" "$OUT/rpm/md-${VERSION}-1.x86_64.rpm"
...
PAGES_BASE="${PAGES_BASE:-https://sihanteng.github.io/md}"
...
<head>
  ...
  <title>md package repositories</title>
  <style>
...
<body>
  <h1>md package repositories</h1>
  <p>Static APT and DNF repos for <a href="https://github.com/SihanTeng/md">md</a>, published from GitHub Releases (no third-party account). Current version: <strong>${VERSION}</strong>.</p>

  <h2>Debian / Ubuntu (APT)</h2>
  <pre>echo "deb [trusted=yes] ${PAGES_BASE}/apt ./" | sudo tee /etc/apt/sources.list.d/md.list
sudo apt update
sudo apt install md</pre>

  <h2>Fedora / RHEL (DNF)</h2>
  <pre>sudo tee /etc/yum.repos.d/md.repo &lt;&lt;'EOF'
[md]
name=md
...
gpgcheck=0
EOF
sudo dnf install md</pre>

  <p><a href="${PAGES_BASE}/apt/">Browse APT</a> · <a href="${PAGES_BASE}/rpm/">Browse RPM</a> · <a href="https://github.com/SihanTeng/md/releases">All releases</a></p>
</body>
</html>
HTML_EOF
